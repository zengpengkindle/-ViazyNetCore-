  ...
  <div class="x-table">
    ...
    <div class="x-table-body">
      <el-table
        ...
        :summary-method="innerGetSummaries"
      ...
      <el-pagination
        ...
        v-model:current-page="lastParams.pageNumber"
        v-model:page-size="lastParams.pageSize"
...
const autoSummaryColumns = computed(() => {
  ...
  if (typeof props.autoSummary === "string")
    return props.autoSummary.split(",");
...
function resize() {
  ...
  pageLayout.value = FULL_PAGE_LATYOU;
  doLayout();
}

onMounted(() => {
  let _prop = props.defaultSort;
  let order: "ascending" | "descending" = props.defaultSortAsc
    ? "ascending"
    : "descending";
...
function onRefresh(force) {
  ...
  const fc =
    typeof props.url === "function"
      ? props.url(lastParams)
      : http.post(props.url, { data: lastParams });
  fc.then(resp => {
    if (test(resp)) {
      lastPage.number = lastParams.pageNumber;
      lastPage.size = lastParams.pageSize;
...
function getParams() {
  const ps = Object.assign({}, props.params);
...
function refresh(current = null, size = null) {
  if (current) lastParams.pageNumber = current;
  if (size) lastParams.pageSize = size;
  onRefresh(true);
}
