...
const item: Ref<CreditModel> = ref({ name: null, key: null, creditType: CreditType.Virtual });
const creditTypes = reactive([{ id: CreditType.ReadyMoney, name: '现金' }, { id: CreditType.Virtual, name: '虚拟货币' }]);
        ...
        <el-form ref="formRef" :model="item" :rules="rules" @submit.native.prevent :validate-on-rule-change="false">
            ...
            <el-form-item label="货币类型">
                <el-radio-group v-model="item.creditType">
                    <el-radio v-for="creditType in creditTypes" :label="creditType.id" :key="creditType.id">{{
