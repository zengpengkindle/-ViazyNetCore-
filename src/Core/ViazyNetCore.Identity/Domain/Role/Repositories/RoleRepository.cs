...
namespace ViazyNetCore.Modules
{
    ...
    public class RoleRepository : DefaultRepository<BmsRole, long>, IRoleRepository
    {
        ...
        public Task<PageData<RoleQueryDto>> FindAllAsync(string nameLike, ComStatus enabled, Pagination pagination)
        {
            return this.Select.WhereIf(nameLike.IsNotNull(), p => p.Name.Contains(nameLike) && p.Status == enabled)
                 .WithTempQuery(p => new RoleQueryDto
                 {
                     Id = p.Id,
        ...
        public Task<List<RoleNameDto>> GetAllAsync()
        {
            return this.Select
                 .WithTempQuery(p => new RoleNameDto
                 {
                     Id = p.Id,
                     Name = p.Name,
                 }).ToListAsync();
