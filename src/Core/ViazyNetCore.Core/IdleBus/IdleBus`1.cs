...
namespace ViazyNetCore.IdleBus
{
    ...
    public partial class IdleBus<TKey, TValue> : IDisposable where TValue : class, IDisposable
    {
        ...
        public TValue Get(TKey key)
        {
            if (isdisposed)
            {
                throw new Exception(string.Format("{0} 实例获取失败，{1} 对象已释放", key, "IdleBus"));
            }

            if (!_dic.TryGetValue(key, out var value))
            {
                Exception ex = new Exception($"{key} 实例获取失败，因为没有注册");
                OnNotice(new NoticeEventArgs(NoticeType.Get, key, ex, ex.Message));
                throw ex;
            }

            DateTime now = DateTime.Now;
            TValue orCreate = value.GetOrCreate();
            if (value.IsRegisterError)
            {
                OnNotice(new NoticeEventArgs(NoticeType.Get, key, null, $"{key} 实例获取失败：检测到注册实例的参数 create 返回了相同的实例，应该每次返回新实例，正确：() => new Xxx()，错误：() => 变量"));
            ...
            OnNotice(new NoticeEventArgs(NoticeType.Get, key, null, string.Format("{0} 实例获取成功 {1}次{2}", key, value.activeCounter, (totalMilliseconds > 5.0) ? $"，耗时 {totalMilliseconds}ms" : "")));
            ThreadScanWatch(value);
            return orCreate;
        }
        ...
        bool InternalRegister(TKey key, Func<TValue> create, TimeSpan? idle, bool isThrow)
        {
            ...
            if (isdisposed)
                throw new Exception($"{key} 注册失败，{nameof(IdleBus<TValue>)} 对象已释放");
        ...
        bool InternalRemove(TKey key, bool isNow, bool isThrow)
        {
            ...
            if (item.value == null) item.lastActiveTime = DateTime.Now.Subtract((TimeSpan)item.idle).AddSeconds(-60); //延时删除
        ...
        void InternalRemoveDelayHandler()
        {
            ...
            foreach (var removeKey in removeKeys)
            {
                ...
                if (DateTime.Now.Subtract((DateTime)removeItem.lastActiveTime) <= removeItem.idle) continue;
                try
                {
