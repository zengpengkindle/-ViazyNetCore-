...
namespace ViazyNetCore.Formatter.Excel
{
    public static class ClosedXmlExtensions
    {
        ...
        public static string GetContentWithOutSpaces(this IXLCell cell, string valueIfNull = "")
        {
            ...
            if(cell.HasFormula && !cell.CachedValue.IsError)
            {
        ...
        {
            if(cell.Value.IsError) return valueIfNull;
        ...
        /// <returns></returns>
        public static T? GetContentTyped<T>(this IXLCell cell, Func<string, T> conversor, T? valueIfNull = default)
        {
            string valor = cell.GetContentWithOutSpaces();
