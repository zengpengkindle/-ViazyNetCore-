...
public class DaprAppApiTokenValidator : IDaprAppApiTokenValidator
{
    ...
    public virtual void CheckDaprAppApiToken()
    {
        ...
        {
            throw new ApiException("Expected Dapr App API Token is not provided! Dapr should set the 'dapr-api-token' HTTP header.");
        }

        if (expectedAppApiToken != headerAppApiToken)
        {
            throw new ApiException("The Dapr App API Token (provided in the 'dapr-api-token' HTTP header) doesn't match the expected value!");
        }
    }
    ...
    {
        return HttpContextAccessor.HttpContext ?? throw new Exception("HttpContext is not available!");
    }
}
