...
{
    public abstract class ObjectStorageServiceBase
    {
        public OSSOptions Options { get; private set; }

        public ObjectStorageServiceBase(OSSOptions options)
        {
        ...
        public virtual async Task<string> PresignedObjectAsync(string bucketName
            , string objectName
            , int expiresInt
        ...
        {
            return "OSS:" + ($"{this.GetType().FullName}_{bucketName}_{objectName}_{type.ToString().ToUpper()}").ToMd5();
        }
    }
}
