    ...
    public class JsonConfigurationParser : IConfigurationParser
    {
        ...
        public IDictionary<string, string> Parse(Stream input, string initialContext)
        {
            ...
            catch (JsonReaderException e)
            {
                ...
                throw new FormatException($"不能解析JSON文本，行号 '{e.LineNumber}': '{errorLine}'.", e);
            }
        ...
        private void VisitToken(JToken token)
        {
            switch (token.Type)
            {
                ...
                default:
                    throw new FormatException($"此'{_reader.TokenType}' JSON 令牌不支持，路径'{_reader.Path}',行'{_reader.LineNumber}',位置'{_reader.LinePosition}'");
            }
        }
