...
namespace ViazyNetCore.Http
{
	...
	public static class CookieCutter
	{
		...
		/// <summary>
		/// Parses a Set-Cookie response header to a CaesarCookie.
		/// </summary>
        ...
        public static string GetValue(IEnumerable<(string Name, string Value)> cookies, string key)
        {
            foreach (var cookie in cookies)
            {
                if (cookie.Name.OrdinalEquals(key,true))
                    return cookie.Value;
            }
		...
		public static bool IsValid(this EasyHttpCookie cookie, out string reason) {
			...
			if (!string.IsNullOrEmpty(cookie.Domain)) {
				if (cookie.Domain.IsIPv4()) {
					reason = "Domain cannot be an IP address.";
					return false;
				}
				if (cookie.OriginUrl.Host.IsIPv4()) {
		...
		// Possible future enhancement: https://github.com/tmenier/Caesar/issues/538
