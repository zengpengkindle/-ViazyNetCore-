...
export function renderSelect(component: ComponentPublicInstance, createOption: (key: string, value: any) => VNode) {
    ...
    const valid = $props.valid || function () { return true };

    for (let key in $props.options) {
        const value = $props.options[key];
        if (valid(key, value))
            children.push(createOption(key, value));
    }
    const slots: Record<string, (props: Record<string, any>) => VNode[]> = {
        default(props) {
            if (component.$slots.default) return [...component.$slots.default(props), ...children];
            return children;
        }
    };
...
export const Select = defineComponent({
    ...
    render() {
        return renderSelect(this, (key, value) => h(ElOption, {
