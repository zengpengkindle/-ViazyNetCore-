<template>
  <div class="select-input">
    <slot name="input" v-bind:show="showDialog">
      <x-input-tag v-if="multiple" v-model="modelValue" :id="id" :label="currentLabel" :placeholder="placeholder"
        @new="showDialog" :readonly="readonly" :max="max" ref="tags" />
      <el-input class="select-input-singleton" v-else :placeholder="placeholder" v-model="display"
        :readonly="label != null" @dblclick.native="showDialog(true)">
        <template #append>
        ...
        <template #suffix>
          <div class="el-input__clear">
            <icon name="circle-close" v-show="modelValue" @click="setValue(null)" title="清除" />
          </div>
        </template>
      </el-input>
    </slot>
    <el-dialog :title="title" v-model="dialogVisible" top="66px" :width="currentDialogWidth" :fullscreen="fullscreen"
      :modal="modal" :append-to-body="true">
      <x-table ref="table" :params="params" :url="url" :border="false">
        <template #form>
          <slot name="form" :params="params" ></slot>
        </template>
        <slot ></slot>
        <el-table-column label="操作">
          <template #default="{ row }">
            <el-button type="text" size="small" @click="handleSelect(row)" v-if="!multiple || !hasValue(row)">请选择</el-button>
          </template>
        </el-table-column>
      </x-table>
    </el-dialog>
...
if (props.modelValue && !props.defaultLabel && (!props.label || props.label == props.id))
...
function handleSelect(row) {
  setValue(row);
  if (!props.multiple || (props.max > 1 && (props.modelValue as any[]).length == props.max)) {
