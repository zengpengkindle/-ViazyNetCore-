...
function convertType(properties, options) {
  ...
  if (properties.hasOwnProperty("$ref")) {
    var t = options.FormatModelName(
      properties.$ref.substring(properties.$ref.lastIndexOf("/") + 1)
    );
    type = {
      TypeOf: "schema",
      TsType: t,
...
function formatData(swagger, options) {
  ...
  // dto对象 / enum对象
  _.forEach(
    isOpenApi ? swagger.components.schemas : swagger.definitions,
    ...
    _.forEach(api, function (md, requestName) {
      ...
      const method = {
        ...
        Responses: convertType(
          md.responses["200"] && md.responses["200"].content
            ? isOpenApi
      ...
      _.forEach(md.parameters, (parameter) => {
        ...
        if (
          pa.Type.Ref &&
          currController &&
          currController.ImportModels.indexOf(pa.Type.Ref) == -1
        ) {
...
function readOpenAPIModel(model, modelArrays, definitions, oas2, index) {
  index = index || 0;
  if (index > 2) return;
  index++;
  if (KUtils.checkUndefined(model) && KUtils.strNotBlank(model)) {
    ...
    if (KUtils.checkUndefined(def)) {
      ...
      let result;
      do {
        if ((result = reg.exec(defStr)) != null) {
          console.log("readOpenAPIModel", result);
          var cmodel = result[1];
          if (KUtils.checkUndefined(cmodel)) {
            if (!modelArrays.includes(cmodel)) {
              modelArrays.push(cmodel);
              readOpenAPIModel(cmodel, modelArrays, definitions, oas2, index);
            }
          }
        }
      } while (result);
...
/**
 * 获取指定接口的swagger 引用类型依赖
 * @param swagger swagger文档
 * @param apiUrl 指定的url路径
 * @param options 方法名的配置，可空
 * @returns schemas
 */
export function findSwaggerDependency(swagger, apiUrl, options) {
    ...
    _.forEach(api, function (md, requestName) {
      ...
      _.forEach(md.parameters, (parameter) => {
        ...
        if (
          ...
          // const d = _.find(apiData.Models, { Name: pa.Type.Ref })
          // if (d) {
          //   _.forEach(d.Properties,(subParameter)=>{
          //       if(subParameter.Type.Ref){
          //           console.log("request ref",subParameter);
          //           currController.ImportModels.indexOf(subParameter.Type.Ref) == -1 &&
          //           currController.ImportModels.push(subParameter.Type.Ref);
          //       }
          //   })
          // }
        }
  ...
  var definitionCopy = {};
  var definitions = swagger["components"]["schemas"];
  console.log("definitions", definitions);
  if (currController.ImportModels && currController.ImportModels.length > 0) {
    var modelArrays = currController.ImportModels;
    if (modelArrays.length > 0) {
      ...
      console.log("modelArrays", modelArrays);
      modelArrays.forEach((model) => {
